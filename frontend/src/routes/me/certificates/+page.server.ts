...
export const load: PageServerLoad = async ({ cookies, fetch }) => {
  ...
  const fulfilledOrders = orders.filter((order: { status: string }) => order.status === 'fulfilled');
  const fulfilledPipes = fulfilledOrders.reduce(
